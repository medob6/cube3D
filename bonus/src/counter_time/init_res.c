...
/*   Updated: 2025/09/13 18:23:56 by omben-ch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_timer_bg_part(t_game *game)
{
	...
	game->timer[TIMER_BG].width = game->timer[TIMER_BG].end_x
		- game->timer[TIMER_BG].start_x;
	game->timer[TIMER_BG].height = game->timer[TIMER_BG].end_y
		- game->timer[TIMER_BG].start_y;
}
...
void	init_min_part(t_game *game)
{
	...
	game->timer[FP_MIN].width = game->timer[FP_MIN].end_x
		- game->timer[FP_MIN].start_x;
	game->timer[FP_MIN].height = game->timer[FP_MIN].end_y
		- game->timer[FP_MIN].start_y;
	...
	game->timer[SP_MIN].width = game->timer[SP_MIN].end_x
		- game->timer[SP_MIN].start_x;
	game->timer[SP_MIN].height = game->timer[SP_MIN].end_y
		- game->timer[SP_MIN].start_y;
...
void	init_point_part(t_game *game)
{
	...
	game->timer[POINT].width = game->timer[POINT].end_x
		- game->timer[POINT].start_x;
	game->timer[POINT].height = game->timer[POINT].end_y
		- game->timer[POINT].start_y;
}
...
void	init_sec_part(t_game *game)
{
	...
	game->timer[FP_SEC].width = game->timer[FP_SEC].end_x
		- game->timer[FP_SEC].start_x;
	game->timer[FP_SEC].height = game->timer[FP_SEC].end_y
		- game->timer[FP_SEC].start_y;
	...
	game->timer[SP_SEC].width = game->timer[SP_SEC].end_x
		- game->timer[SP_SEC].start_x;
	game->timer[SP_SEC].height = game->timer[SP_SEC].end_y
		- game->timer[SP_SEC].start_y;
...
void	init_timer_pic(t_game *game)
{
	...
	game->img_timer.img = mlx_new_image(game->mlx, game->win_timer_w,
			game->win_timer_h);
	game->img_timer.addr = mlx_get_data_addr(game->img_timer.img,
			&game->img_timer.bpp, &game->img_timer.line_len,
			&game->img_timer.endian);
