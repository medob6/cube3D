...
/*   Updated: 2025/09/16 14:43:25 by omben-ch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_val_nwesfc(t_fcub *fcub, int fd)
{
	...
	while (check_struct_fcub_val(fcub))
	{
		...
		{
			free(line);
			print_error_argument("North or South or East \
or West or Floor or Ceiling");
			return (1);
		}
		if (line[0] != '\n' && check_nwesfc(fcub, line))
		{
			free(line);
			print_error_argument("North or South or East or \
West or Floor or Ceiling");
			return (1);
		}
		free(line);
	}
